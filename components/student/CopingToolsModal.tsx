...
const CopingToolsModal: React.FC<CopingToolsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleToolClick = (toolName: string) => {
    if (toolName === 'Guided Breathing' || toolName === 'Mindfulness Games' || toolName === 'Journaling Prompts') {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      {activeTool === 'Journaling Prompts' ? (
        <JournalingPrompts onBack={handleBack} />
      ) : (
        <div className="bg-brand-background rounded-[2rem] shadow-2xl w-full max-w-2xl min-h-[500px] h-auto max-h-[90vh] flex flex-col relative animate-fade-in-up">
